...
- **Scripts de población (CLI del backend, `python -m app.cli`)**: la lógica de población/enriquecimiento de `endgame_lessons` vive en `app/services/endgame_admin_service.py` y se invoca desde el CLI `app/cli.py` (no hay scripts sueltos en `scripts/` para este módulo). Comandos:
