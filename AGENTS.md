...
- **IA centralizada con proveedores**: todo el IA usa el singleton `gemini_client` en `app/services/gemini_client.py`, que orquesta una cadena de failover: **Gemini primario** (`gemini-3.7-flash`) → **Gemini reserva** (`gemini-3.1-flash-lite`; la anterior `gemini-2.5-flash-lite` quedó retirada por Google y devolvía 404) → **Groq** (`openai/gpt-oss-120b`, proveedor de respaldo gratuito vía `app/services/grok_client.py`). Reintentos con backoff 1s/2s/4s y failover automático ante 503; solo modelos vigentes — los retirados por Google dan 404. Si TODO agota, se lanza `GeminiSaturadoError` (503 con mensaje amigable). **Groq se omite silenciosamente** si `GROQ_API_KEY` está vacía; key gratis en `console.groq.com` (1.000 req/día). El catálogo de Groq cambia: el antiguo `llama-3.3-70b-versatile` ya fue retirado de cuentas nuevas (404); verifica los modelos reales con `GET /models`. **No inicialices `genai.GenerativeModel`** ni dupliques `genai.configure` en los servicios; llama `gemini_client.generate_json(...)` / `generate_text(...)` (los 6 servicios consumidores no cambian: el failover es transparente). El timeout (`settings.GEMINI_TIMEOUT_SECONDS`, default 30) se aplica en `GeminiClient._generate` (bajado de 120 para no bloquear threads del threadpool cuando Gemini tarda); Groq usa `GROQ_TIMEOUT_SECONDS`.
...
- **Endpoints** (`app/api/v1/endgames.py`, prefijo `/endgames`): `GET /lessons` (catálogo por `category` + estado de progreso), `GET /lessons/{slug}` (`initial_fen`, `audio_url`, `podcast_script`, `timeline_events`), `POST /lessons/{slug}/progress`, `POST /stockfish-move` (devuelve `{move_uci, move_san, fen_after}`, sin auth). Parámetros: `skill_level` (1-20, default 8), `time_limit` (0.05-2.0 s, default 0.5) y **`depth`** (1-15, opcional; si se provee, tiene prioridad sobre `time_limit` — se usa con `chess.engine.Limit(depth=...)`, verificado con motor real). Lado frontend: `getStockfishMove(fen, skillLevel, timeLimit, depth?)` en `src/lib/api.ts`.
...
### Análisis: modos IA vs autodiagnóstico y claridad del error

- **Campo `analysis_mode`** en `UserGameAnalysis` (y en el payload `GameAnalysisCreate`/`GameAnalysisDraftCreate`): `"auto"` (detecta si hay comentarios), `"ai"` (análisis completo del GM **sin** comentarios del alumno) o `"self_audit"` (audita el autodiagnóstico del alumno). La detección la hace `tutor_service._resolve_mode` (formulario vacío → `ai`; con contenido → `self_audit`). Se persiste en `analysis_mode` y se expone en `GameAnalysisResponse`.
- **Prompts bifurcados** en `app/services/tutor_service.py`: `_get_ai_system_prompt` (análisis maestro completo de la partida cuando no hay comentarios) y `_get_audit_system_prompt` (auditoría del autodiagnóstico). Ambos devuelven el mismo esquema `GeminiFeedback` (4 bloques) pero el de auditoría exige claridad en el error.
- **Claridad del error**: `GeminiFeedback.auditoria_conclusiones.razon_insuficiente` es un campo obligatorio y detallado (3-5 frases) **solo cuando `plan_correcto` es `false`**; explica qué afirmó el alumno, qué ocurre realmente, la jugada/concepto concreto y por qué es insuficiente. Los demás bloques se limitan a 2-3 frases (la regla de concisión NO se aplica a `razon_insuficiente`). El frontend lo muestra en `GeminiFeedbackDisplay` con un bloque rojo "Por qué es insuficiente".
- **Apertura con ECO**: en ambos modos, `feedback_fases.apertura` debe citar PRIMERO el **código ECO y el nombre de la apertura en español** (p. ej. `ECO C65 – Ruy López`). En la auditoría es parte de la corrección y **NO se pide al alumno**.
- **UI (frontend)**: `AnalysisFormPanel` tiene dos acciones. El botón **"Análisis por IA (sin comentarios)"** solo aparece en **partidas propias** (`gameType === "USER"`); en **partidas de GM** se oculta porque el objetivo es el autodiagnóstico del usuario (queda solo "Enviar a Evaluación del Gran Maestro"). El envío vacío en GM sigue cayendo en modo `ai` (conserva el análisis completo del GM), pero no se anuncia como botón. `GeminiFeedbackDisplay` distingue el badge "Analizado por IA" del veredicto correcto/incorrecto según `mode`.
- **Migración**: `alembic/versions/f0a1b2c3d4e5_add_analysis_mode_to_user_game_analyses.py` añade la columna `analysis_mode` (idempotente; `app/main.py` aplica `alembic upgrade head` al arrancar).
...
- **Partidas Guiadas de Apertura** (`/estudio-aperturas`):
  ...
  - **Elección tras enviar al GM**: el alumno elige entre **"Finalizar Estudio"** (ver el informe) o **"Continuar Jugando contra Stockfish"** desde el `out_of_theory_fen` (**`OpeningStockfishBoard.tsx`**, columna 55%) mientras el plan se audita en segundo plano; un banner avisa cuando el informe queda listo.
  - **Guardado del historial (la partida SIEMPRE se guarda al salir del tablero Stockfish)**: el componente sube el PGN completo (apertura + medio juego) y el resultado real al padre vía `onPgnChange(pgn, result)` en cada jugada. Tres caminos posibles:
    - **"Guardar partida y ver informe"** → `onSave()`: guarda con el PGN/resultado real ya notificados y sale.
    - **"Abandonar partida (gana el motor) y guardar"** → `onAbandon(pgn)`: el componente construye un PGN con derrota forzada del usuario (`0-1` si juega blancas, `1-0` si negras) y lo registra en el histórico antes de salir.
    - **Fin natural** (mate/ahogado/50 jugadas/triple repetición) → `onGameEnded(pgn, result)`: se dispara UNA vez vía `finalize` y la página auto-guarda el resultado real (incluye la jugada final, porque `finalize` recibe `latestSan` explícito y lo añade a `continuationMoves` para no quedarse corto por la asincronía del estado).
  - **Dificultad de Stockfish**: selector `SKILL_LEVELS` (3/8/13/18) + backend acepta `depth` (1-15) con prioridad sobre `time_limit` (ver `endgames.py`).
- **Histórico desdoblado** (`AnalysisHistoryList.tsx`): las partidas de apertura (`analysis_mode === "guided_opening"`) se separan del resto en un selector segmentado **"Todos / Estudio de Aperturas / Otras partidas"** con contadores; mantienen el badge "PARTIDA GUIADA".
...
- **Prop de `OpeningStockfishBoard` (patrón de refs no reactivos)**: `fullPgn` y `onPgnChange` recalcularon vía `useEffect` sobre un `useMemo`; `resultRef`/`gameRef` son refs (no disparan recalculo) que se "leen" como resultado real tras cada re-render provocado por `setPosition`. Como `continuationMoves`/estado son asíncronos, **cualquier PGN que deba reflectir la jugada final se construye con la lista SAN explícita** (`buildCompletePgn(sanList, result)`), no con `continuationMoves` a secas en el mismo tick.
...
cmd /c "cd frontend && npm test"        # Jest + Testing Library (~207 tests)
```
...
- **Exportar a PDF del histórico** (`historico/[analysisId]` → botón "Exportar a PDF" = `window.print()`): el informe imprimible es `PrintAnalysisReport` (`hidden print:block` en `globals.css`). **No uses `break-inside: avoid` (`print-block`) en la sección de evaluación del GM**: es más alta que una página y Chromium omite todo el bloque en la impresión (bug conocido de `break-inside: avoid` + flex/grid). La sección de evaluación del GM NO lleva `print-block` y fluye libremente. En modo `ai` se oculta el bloque "Comentarios del Usuario" (el análisis IA va sin comentarios del alumno) y solo sale la evaluación del GM. El export usa el `feedback` en vivo (mismo que en pantalla), no solo `initialFeedback`. Test de regresión: `PrintAnalysisReport.print.test.tsx`.
...
- **Warnings `react-hooks/exhaustive-deps` (limeza ago 2026)**: en tableros con `gameRef`/`resultRef` no reactivos se usa `fen`/`position` como **señal reactiva** en las deps aunque no se lean en el cuerpo (p. ej. `isUserTurn`). El warning se reporta en la línea del array de deps (cierre del hook), por lo que `// eslint-disable-next-line` **no** funciona: hay que envolver cada hook con comentario de bloque `/* eslint-disable react-hooks/exhaustive-deps */` … `/* eslint-enable */` y dejar un comentario explicativo. Ya aplicado en `LiveGameBoard.tsx`, `EndgamePracticeBoard.tsx`, `OpeningStockfishBoard.tsx`, `InteractiveChessBoard.tsx`, `AnalysisFormPanel.tsx` e `InteractiveDemo.tsx`. El build quedar limpio es el criterio (`npm run build`).
